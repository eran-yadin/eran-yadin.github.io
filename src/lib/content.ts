...
export async function getPosts(): Promise<Post[]> {
  const posts = await getCollection('posts', (e) => showDrafts || !e.data.draft);
  for (const p of posts) p.data.title ??= titleFromId(p.id);
  return posts.sort((a, b) => tended(b).valueOf() - tended(a).valueOf());
}
...
/** "my-first-post" -> "My first post" (used when a post has no title: field). */
...
export function tended(n: Post): Date {
...
/** All tags across posts and projects, with counts. */
export async function getTags(): Promise<Map<string, number>> {
  const all: Entry[] = [...(await getPosts()), ...(await getProjects())];
...
const WIKI = /\[\[([^\]|#]+?)(?:#[^\]|]+)?(?:\|[^\]]+)?\]\]/g;
const MD_LINK = /\]\((\/(?:posts|projects)\/[^)#\s]+?)\/?(?:#[^)]*)?\)/g;
